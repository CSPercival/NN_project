...
for epoch in range(0, 2000):
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), f"results/yolo_weights_epoch_{epoch}.pth")
    
    with open(f"results/train_losses_{epoch}.json", "w") as f:
        json.dump([float(loss) for loss in train_losses], f)
        
    with open(f"results/val_losses_{epoch}.json", "w") as f:
...
new_weights_path = f"results/yolo_weights_{next_idx}.pth"
...
new_losses_path = f"results/train_losses_{next_idx}.json"
with open(new_losses_path, "w") as f:
    json.dump([float(loss) for loss in train_losses], f)
    
with open(f"results/val_losses_{next_idx}.json", "w") as f:
    json.dump([float(loss) for loss in val_losses], f)
    
print(f"--> [AUTO-SAVE] Kolejne wartości błędu z treningu zapisane do: {new_losses_path}")
